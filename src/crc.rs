// Module that handles computing CRC16 values

pub fn compute_crc(buffer: &Vec<u8>) -> u16 {
    use std::io::Cursor;
    use byteorder::{LittleEndian, ReadBytesExt};

    // Lookup tables for 8 x 256 64 bit CRC 16 computation algorithm
    // TODO: add generation program 
    static TABLE0: [u16; 256] = [
    ...
    while stream_length - pos > 7 {
        ...
        let index7: usize = ((value & 0xff) as usize) ^ ((crc & 0xff) as usize);
        let index6: usize = (((value >> 8) & 0xff) as usize) ^ (((crc >> 8) & 0xff) as usize);
        let index5: usize = ((value >> 16) & 0xff) as usize;
        let index4: usize = ((value >> 24) & 0xff) as usize;
        let index3: usize = ((value >> 32) & 0xff) as usize;
        let index2: usize = ((value >> 40) & 0xff) as usize;
        let index1: usize = ((value >> 48) & 0xff) as usize;
        let index0: usize = (value >> 56) as usize;
        crc = TABLE7[index7] ^ 
            TABLE6[index6] ^
            TABLE5[index5] ^
            TABLE4[index4] ^
